...
const iconMap: Record<string, any> = {
  Servers: Server, Desktops: Monitor, Laptops: Laptop, Printers: Printer,
  "UPS / Inverters": Battery, Scanners: ScanLine, Photocopiers: Copy,
  "ID Card Printers": CreditCard, Accessories: Plug, "POS Terminals": ShoppingCart,
  "Printer Consumables": Package,
};
      ...
      <section ref={ref} className="section-padding relative overflow-hidden">
        ...
        <div className="container mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7">
            {products.map((p) => {
              const IconComponent = iconMap[p.title] || Package;
              return (
                <div key={p.id} className="gsap-stagger group relative bg-card rounded-3xl overflow-hidden border border-border/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10">
                  {p.badge && (
                    <div className="absolute top-4 left-4 z-20 bg-secondary text-secondary-foreground text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-lg">{p.badge}</div>
                  )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-2">
                      <a href={getWhatsAppLink(p.title, p.price)} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[#25d366] text-primary-foreground text-xs font-semibold hover:bg-[#20bd5a] transition-all duration-300 shadow-lg shadow-[#25d366]/20 hover:-translate-y-0.5">
                        <MessageCircle className="w-3.5 h-3.5" /> Order Now
                      </a>
                      <Link to="/contact" className="flex items-center justify-center gap-1 px-4 py-2.5 rounded-xl border border-border text-xs font-semibold text-foreground hover:border-primary hover:text-primary transition-all duration-300">Enquire</Link>
                    </div>
                  </div>
                </div>
              );
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="container mx-auto text-center relative z-10 px-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hello CIS Limited, I need a custom quote for bulk IT equipment.")}`} target="_blank" rel="noopener noreferrer" className="btn-pill bg-[#25d366] text-primary-foreground text-sm shadow-xl hover:-translate-y-1 transition-transform">
              <MessageCircle className="w-4 h-4" /> WhatsApp Us
            </a>
            <Link to="/contact" className="btn-pill-light text-sm">Request Quote <ArrowRight className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>
    </div>
