...
import { MapPin, Phone, Mail, Send, ArrowRight, Clock, MessageCircle } from "lucide-react";
          ...
          <div className="grid lg:grid-cols-2 gap-14">
            <div>
              ...
              <div className="gsap-fade-up img-overlay rounded-3xl h-52 shadow-xl mb-8 overflow-hidden" style={{ borderRadius: '20px 80px 20px 80px' }}>
                <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=600&q=80" alt="Contact us" />
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
                  {[
                    ...
                    <div key={field.key}>
                      ...
                      <input type={field.type} required value={form[field.key as keyof typeof form]} onChange={e => setForm({ ...form, [field.key]: e.target.value })} className="w-full px-5 py-3.5 rounded-xl border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300" placeholder={field.placeholder} />
                    </div>
                  ))}
                  <div>
                    <label className="block text-sm font-semibold text-foreground mb-2 font-display">Message</label>
                    <textarea required rows={4} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} className="w-full px-5 py-3.5 rounded-xl border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-300 resize-none" placeholder="Tell us about your project..." />
                  </div>
                  <button type="submit" className="w-full btn-pill-primary justify-center shadow-xl shadow-primary/20"><Send className="w-4 h-4" /> Send Message</button>
                </form>
              </div>
            </div>
          </div>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="container mx-auto text-center relative z-10 px-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a href="https://wa.me/23417616165" target="_blank" rel="noopener noreferrer" className="btn-pill bg-[#25d366] text-primary-foreground text-sm shadow-xl hover:-translate-y-1 transition-transform"><MessageCircle className="w-4 h-4" /> WhatsApp Us</a>
            <a href="tel:234-1-7616165" className="btn-pill-light text-sm"><Phone className="w-4 h-4" /> Call Now</a>
          </div>
        </div>
      </section>
    </div>
